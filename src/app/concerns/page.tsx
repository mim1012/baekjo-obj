...
export default async function ConcernsPage() {
  ...
  const published = await getPublishedPageContent<ConcernsContent>('concerns').catch((error: unknown) => {
    logServerError('[Concerns] CMS 조회 실패', error);
    return null;
  });
  ...
  if (!managed) {
    try {
      ...
      logServerError('[Concerns] 기존 페이지 문구 조회 실패', error);
    }
  }
    ...
    <main className="flex flex-col bg-[#F8F6F0] w-full" data-cms-managed={managed ? 'concerns' : undefined}>
      ...
      {content.hero.visible && <section className="relative h-[640px] w-full overflow-hidden bg-[#EDE5D8] sm:h-[620px] md:h-[480px] lg:h-[520px] xl:h-[560px]">
        {content.hero.image && <Image
          ...
          quality={90}
          priority
          className="object-cover object-[var(--concerns-hero-position)] md:object-center"
          style={{ '--concerns-hero-position': content.hero.imagePosition } as CSSProperties}
        />}
        <div
          aria-hidden="true"
        ...
        <section className="mb-[48px] md:mb-[64px]">
          <div className="flex flex-col md:flex-row bg-[#16382D] rounded-[20px] md:rounded-[24px] overflow-hidden h-auto md:h-[220px] lg:h-[260px]">
            ...
            <div className="flex items-center justify-start md:justify-center w-full md:w-[20%] px-[32px] pb-[32px] md:p-0">
              <Link href={content.insurance.buttonHref} className="inline-flex items-center justify-center px-6 py-3.5 bg-white text-[#16382D] text-[14px] font-bold rounded-[12px] hover:bg-[#F8F6F0] transition-colors whitespace-nowrap">
          ...
          <div className="overflow-hidden rounded-[20px] border border-[#E4DDD1] bg-white">
            <div className="w-full p-[28px] md:p-[34px]">
              ...
              <div className="flex flex-col gap-2 md:gap-2.5">
                {content.faq.items.filter((faq) => faq.visible).map((faq, idx) => (
