...
interface ReviewsPageContent extends Record<string, unknown> {
  hero: { eyebrow: string; title: string };
  stats: { totalLabel: string; ratingLabel: string; photoLabel: string; countSuffix: string };
  filters: Array<{ value: string; label: string; visible: boolean }>;
...
export default async function ReviewsPage({
  ...
  const [{ items }, content] = await Promise.all([
    ...
    getPublishedPageContent<ReviewsPageContent>('reviews'),
  ]);
    ...
    <div className="min-h-dvh bg-[#F4F2EC] bg-noise py-10 lg:py-12">
      <div className="mx-auto max-w-[1280px] px-4 sm:px-8 lg:px-10">
        ...
        <div className="hide-scrollbar mb-6 flex snap-x snap-mandatory scroll-px-1 gap-2 overflow-x-auto pb-1" aria-label="후기 필터">
          {content.filters.filter((item) => item.visible).map(({ value: id, label }) => (
            <Link
              key={id}
