...
import { selectReviewsContent, type ReviewsContent } from '@/lib/cms/source/reviews';
...
export default async function ReviewsPage({
  ...
  const published = await getPublishedPageContent<ReviewsContent>('reviews').catch((error: unknown) => {
    logServerError('[Reviews] CMS 조회 실패', error);
    return null;
  });
  ...
  if (!managed) {
    try {
      ...
      logServerError('[Reviews] 기존 페이지 문구 조회 실패', error);
    }
  }
  const content = selectReviewsContent(published, settings);
      ...
      <div className="mx-auto max-w-[1280px] px-4 sm:px-8 lg:px-10">
        <div className="mb-5 flex flex-col gap-4 border-b border-[#D8D6CE] pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-[30px] font-bold leading-[1.15] tracking-tight text-[#17211D] md:text-[42px]">{content.hero.title}</h1>
          </div>
        </div>
        ...
        <div className="mb-6 grid grid-cols-3 divide-x divide-[#E7E0D5] border-b border-[#E7E0D5] bg-white">
          <div className="flex min-h-[88px] flex-col justify-center px-3 py-3 sm:px-5">
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6F766F] sm:text-[11px]">{content.stats.totalLabel}</div>
            <div className="text-[24px] font-bold leading-none tracking-tight text-[#17211D] sm:text-[28px]">{reviews.length}<span className="ml-1 text-[12px] font-medium text-[#6F766F]">{content.stats.countSuffix}</span></div>
          </div>
          <div className="flex min-h-[88px] flex-col justify-center px-3 py-3 sm:px-5">
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6F766F] sm:text-[11px]">{content.stats.ratingLabel}</div>
          ...
          <div className="flex min-h-[88px] flex-col justify-center px-3 py-3 sm:px-5">
            <div className="mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6F766F] sm:text-[11px]"><ImageIcon className="size-3.5 text-[#A8742E]" aria-hidden="true" />{content.stats.photoLabel}</div>
            <div className="text-[24px] font-bold leading-none tracking-tight text-[#17211D] sm:text-[28px]">{photoReviewsCount}<span className="ml-1 text-[12px] font-medium text-[#6F766F]">{content.stats.countSuffix}</span></div>
          </div>
        </div>

        {/* 필터 */}
        <div className="hide-scrollbar mb-6 flex snap-x snap-mandatory scroll-px-1 gap-2 overflow-x-auto pb-1" aria-label="후기 필터">
          {content.filters.filter((item) => item.visible).map(({ value, label }) => (
            <Link
              key={value}
              href={value === 'all' ? '/reviews' : `/reviews?filter=${value}`}
              className={`flex h-11 shrink-0 snap-start items-center rounded-md border px-4 text-[13px] font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#A8742E] focus-visible:ring-offset-2 ${filter === value ? 'border-[#17211D] bg-[#17211D] text-[#FBFAF7]' : 'border-[#D8D6CE] bg-white text-[#59615B] hover:border-[#A8742E] hover:bg-[#F3EEE6] hover:text-[#17211D]'}`}
            >
              {label}
            </Link>
          ))}
