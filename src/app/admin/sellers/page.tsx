...
export default function SellersPage() {
  ...
  const columns = [
    ...
    { key: 'registration', header: '사업자 정보', render: (seller: Seller) => <div className="text-xs leading-5"><p>{seller.businessRegistrationNumber}</p><p className="text-gray-500">{seller.mailOrderRegistrationNumber || '통신판매업 신고번호 미입력'}</p></div> },
    { key: 'contact', header: '고객 연락처', render: (seller: Seller) => <div className="text-xs leading-5"><p>{seller.phone || '연락처 미입력'}</p><p className="text-gray-500">{seller.email || '이메일 미입력'}</p></div> },
    { key: 'shipping', header: '배송 정책', render: (seller: Seller) => <div className="text-xs leading-5"><p>기본 {!seller.shippingFee ? '무료' : formatPrice(seller.shippingFee)}</p><p className="text-gray-500">{seller.freeShippingThreshold === undefined ? '무료배송 기준 없음' : `${formatPrice(seller.freeShippingThreshold)} 이상 무료`}</p></div> },
