...
const TASK_LINKS = [
  ...
  ['고민별 설명 글과 관련 상품을 바꿔요', '고민·케어 가이드', '/admin/concerns', '상품 카드와 다른 콘텐츠입니다.'],
  ['홈 배너·바로가기·홈 문구를 바꿔요', '전체 화면 관리(홈)', '/admin/pages/home', '이 화면을 활성화하기 전에는 환경 설정 화면이 대신 사용됩니다.'],
...
export default function AdminGuidePage() {
  return (
    <div className="space-y-8 pb-16">
      <PageHeader title="관리자 사용 안내" description="처음 온 직원이 한 번 교육받은 뒤 혼자 운영할 수 있도록 만든 화면입니다. 작업은 항상 현재 고객 홈페이지 화면에서 시작하고, 홈페이지 자체의 디자인이나 내용을 임의로 다시 만들지 않습니다.">
        <Link href="/admin/pages" className="flex min-h-11 items-center gap-2 rounded bg-[#17201B] px-4 text-sm font-semibold text-white">
      ...
      <div className="border border-[#C7D5C9] bg-[#EEF4EE] px-5 py-4 sm:px-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-pretty text-xs leading-5 text-[#506057]">첫 교육에서는 상품 1개 수정, 화면 문구 1개 임시저장·게시, 테스트 항목 1개 숨김을 직접 해보면 됩니다.</p>
          </div>
        </div>
      </div>

      <FormSection title="모든 작업의 공통 순서" description="메뉴를 외우지 말고 이 순서만 지킵니다.">
      ...
      <FormSection title="하려는 일로 메뉴 찾기" description="왼쪽 메뉴 이름이 익숙하지 않아도 이 표에서 바로 이동할 수 있습니다.">
        <div className="divide-y divide-[#E7E0D5]">
          {TASK_LINKS.map(([task, menu, href, explanation]) => (
            <div key={task} className="grid gap-3 py-4 first:pt-0 sm:grid-cols-[minmax(0,1.3fr)_minmax(0,1.5fr)_auto] sm:items-center">
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <StatusBadge status="neutral" label={menu} />
              </div>
              <Link href={href} className="flex min-h-10 items-center gap-2 rounded border border-[#D1D0C8] px-3 text-xs font-semibold text-[#17201B]">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <FormSection title="버튼과 상태의 정확한 뜻" description="어디에 반영되는지는 각 관리 화면 상단과 입력칸 아래에도 표시됩니다.">
          <dl className="divide-y divide-[#E7E0D5]">
            {TERMS.map(([term, meaning]) => (
              <div key={term} className="grid gap-1 py-4 first:pt-0 sm:grid-cols-[100px_1fr] sm:gap-4">
        ...
        <div className="space-y-6">
          <FormSection title="헷갈리기 쉬운 상품 용어" description="이 두 가지는 서로 다른 곳에 사용됩니다.">
            <div className="divide-y divide-[#E7E0D5]">
              <div className="py-4 first:pt-0">
                <p className="text-sm font-bold text-[#17211D]">상품 카드 고민 태그</p>
                <p className="mt-1 text-pretty text-xs leading-5 text-[#6F766F]">배변·생활·피부처럼 상품 카드 가격 아래에 보이는 짧은 말입니다.</p>
              </div>
              <div className="py-4">
