...
function ShopContent() {
  ...
  const visibleProducts = products.filter(
    (product) => product.isVisible !== false && visibleBrandIds.has(product.brandId),
  );
  ...
  const recommendedProducts = visibleProducts.filter(p => p.isRecommended || p.isBest).slice(0, 4);
