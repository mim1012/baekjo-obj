                  ...
                  <div key={`${data.order.id}-${idx}`} className="mypage-card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex gap-4">
                      {canOpenProduct ? (
                        ...
                        <div className="relative h-[72px] w-[72px] shrink-0 overflow-hidden rounded-lg border border-[#EBE6DC] bg-white">
                          {product?.image && (
                            <Image src={product.image} alt={data.item.productName} fill className="object-cover" />
                          )}
                        </div>
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        {canOpenProduct ? (
                          ...
                          <span className="text-sm font-semibold text-[#18231F] line-clamp-1">{data.item.productName}</span>
                        )}
                    ...
                    {product && (
                      <button
                        onClick={() => onWriteReview(product, data.order.id, undefined, data.item.optionName)}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex gap-3">
                        {canOpenProduct ? (
                          ...
                          <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-[#EBE6DC] bg-white">
                            {product?.image && (
                              <Image src={product.image} alt={product.name} fill className="object-cover" />
                            )}
                          </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {product && (
                          <button
                            onClick={() => onEditReview(review, product, orderItem?.optionName)}
