...
const numberedTitleDescriptionItemFields: CmsItemFieldDefinition[] = [
  { key: 'number', label: '순서 표시', type: 'text', placeholder: '01' },
  { key: 'title', label: '제목', type: 'text' },
  { key: 'description', label: '설명', type: 'textarea' },
  { key: 'visible', label: '표시', type: 'boolean', defaultValue: true },
...
const audit: CmsPageDefinition = {
  ...
  sections: [
    { id: 'hero', label: '첫 화면', description: 'Audit 화면에 들어오면 가장 먼저 보이는 영역입니다.', fields: [...heroFields, text('hero.imageCaptionLine1', '이미지 캡션 첫 줄'), text('hero.imageCaptionLine2', '이미지 캡션 둘째 줄')] },
    { id: 'checkpoints', label: '검토 기준', description: '검토 기준 카드의 문구와 순서를 관리합니다.', fields: [toggle('checkpoints.visible', '영역 표시'), text('checkpoints.ariaLabel', '접근성 영역 이름'), text('checkpoints.eyebrow', '작은 영문 제목'), text('checkpoints.title', '제목'), textarea('checkpoints.description', '설명'), items('checkpoints.items', '검토 기준 카드', numberedCardItemFields)] },
    { id: 'process', label: '진행 과정', description: 'Audit이 계속 보완되는 과정과 순서를 관리합니다.', fields: [toggle('process.visible', '영역 표시'), text('process.ariaLabel', '접근성 영역 이름'), text('process.eyebrow', '작은 영문 제목'), text('process.title', '제목'), textarea('process.description', '설명'), items('process.items', '진행 단계', numberedTitleDescriptionItemFields)] },
    { id: 'status', label: '표시 상태', description: '완료·확인 중·업데이트 예정 안내를 관리합니다.', fields: [toggle('status.visible', '영역 표시'), text('status.ariaLabel', '접근성 영역 이름'), text('status.eyebrow', '작은 영문 제목'), text('status.title', '제목'), textarea('status.description', '설명'), items('status.items', '상태 카드', titleDescriptionItemFields), textarea('status.notice', '상태 안내 문구'), textarea('status.disclaimer', '구매 전 주의 안내'), textarea('status.legalDisclaimer', 'Audit 법적 안내')] },
...
const b2b: CmsPageDefinition = {
  ...
  defaultContent: {
    hero: { visible: true, eyebrow: 'BAEKJO OBJET FOR BUSINESS', title: '반려가족과 만나는 순간을\n함께 설계합니다.', description: '백조오브제 B2B는 기관과 브랜드의 목적에 맞춰 상품과 콘텐츠, 필요한 구성을 함께 제안합니다.', image: '/images/b2b-partnership-hero-v1.webp', imageAlt: '반려생활 기관을 위한 백조오브제 B2B 파트너십', primaryCtaLabel: 'B2B 문의하기', primaryCtaHref: '/landing/care-kit#partner', secondaryCtaLabel: '협업 프로그램 보기', secondaryCtaHref: '#programs', overlayEyebrow: 'Care in every touchpoint', overlayText: '기관의 목적과 보호자의 필요가 만나는 구성을 제안합니다.' },
    partners: {
