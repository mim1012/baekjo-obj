 ...
 *
 * 게시형(CMS) 관리 버튼은 cmsPageKey로 src/lib/cms/pageDefinitions.ts의 CMS_PAGE_DEFINITIONS와
 * 1:1 연결된다 — 실제 편집 화면은 /admin/pages/[pageKey](PR2 CMS)이며, 이 파일이 그 목록을
 * 다시 구현하지 않는다. 아직 실제 관리자 화면이 없는 기능은 화면이 만들어질 때까지 이
 * 기준표에 등록하지 않는다(가짜 링크 금지).
 */
...
export const PUBLIC_PAGE_REGISTRY: PublicPageRegistryEntry[] = [
  ...
  },
  {
    ...
    actions: [
      content(
        '홈 화면 문구 편집',
        'home',
        '히어로·쇼핑 카테고리·오늘의 추천·큐레이션·Audit·솔루션·후기 영역 문구를 게시 방식으로 관리합니다. 이 화면에서 활성화하기 전에는 환경 설정(구 편집기)이 대신 사용됩니다.',
      ),
      manager('홈 화면 편집(구)', '/admin/settings', '페이지 관리에서 홈을 아직 활성화하지 않았을 때 쓰는 이전 편집기입니다.', ['등록', '수정', '삭제', '노출', '순서', '게시']),
  ...
  },
  {
    ...
    title: '소셜가입 정보 완성',
    publicRoute: '/auth/complete-profile',
    previewRoute: '/login',
    routePattern: '/auth/complete-profile',
    group: '주문·회원',
    screenType: '기능 화면',
    description: '소셜 로그인 회원이 주문에 필요한 이름과 휴대전화번호를 입력하는 화면입니다.',
    editableAreas: ['회원 이름·휴대전화번호', '회원 활성 상태'],
    systemAreas: ['로그인 세션', '필수 입력 검증', '완료 후 복귀 주소'],
    actions: [operation('회원 정보 확인', '/admin/members', '저장된 회원 정보와 계정 상태를 확인합니다.', ['조회', '상태'])],
  },
  {
    key: 'password-help',
    title: '비밀번호 찾기·재설정',
