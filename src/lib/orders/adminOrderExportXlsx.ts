...
function itemRow(order: Order, item: OrderItem, index: number, brandMap: ReadonlyMap<string, Brand>, seenBrands: Set<string>): (string | number)[] {
  ...
  const firstBrandRow = !seenBrands.has(brandKey);
...
export async function serializeAdminOrdersXlsx(orders: readonly Order[], brands: readonly Brand[]): Promise<ArrayBuffer> {
  ...
  for (const order of orders) {
    ...
    items.forEach((item, index) => {
      const row = itemRow(order, item, index, brandMap, seenBrands);
      const brandKey = item.brandId ?? '__legacy__';
      const entry = rowsByBrand.get(brandKey) ?? { rows: [], quantity: 0, productAmount: 0, shipping: 0, finalAmount: 0, name: String(row[2] ?? '') };
      entry.rows.push(row);
    ...
    for (const entry of rowsByBrand.values()) {
      ...
      const subtotal = sheet.addRow(['', '', `${entry.name} 합계`, '', '', entry.quantity, entry.productAmount, entry.shipping, entry.finalAmount, '', '', '', '', '']);
