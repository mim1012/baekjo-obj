...
import { ReviewViewItem, InquiryViewItem, User, Order } from '@/types';
import { getMergedReviews, getMergedInquiries } from '@/lib/adapters';
import { getCurrentUser, getOrders, STORAGE_EVENTS, addProductReview, addProductInquiry, buildReviewTargetKey } from '@/lib/storage';
...
export default function ProductTabsClient({ product, children }: ProductTabsClientProps) {
  ...
  const [writableItems, setWritableItems] = useState<any[]>([]);

  const loadData = () => {
    const currentUser = getCurrentUser();
    setUser(currentUser);
    if (currentUser) {
      setOrders(getOrders().filter(o => o.customerName === currentUser.name));
    }
