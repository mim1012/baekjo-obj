...
export default function ProductDetailClient({ product }: Props) {
  ...
  // ?ÅÌíà ?ÑÌôò ??Î°úÏª¨ state ?¨ÎèôÍ∏∞Ìôî(?¥Ï†Ñ ?ÅÌíà???∏Îç±?§¬∑Ïàò???îÏ°¥ Î∞©Ï?)
  // ??effect ???ôÍ∏∞ setState ??lint(cascading render) ?êÎü¨???åÎçî ?®Í≥Ñ Î¶¨ÏÖã ?®ÌÑ¥ ?¨Ïö©.
  const [prevProductId, setPrevProductId] = useState(product.id);
  if (prevProductId !== product.id) {
    setPrevProductId(product.id);
    setActiveImage(0);
    setQuantity(1);
    // ?åÏÉù Í≤ÄÏ¶?validOption)ÎßåÏúºÎ°??§Î•∏ ?ÅÌíà??Í∞ôÏ? ?µÏÖò id Î•??????¥Ï†Ñ ?†ÌÉù???†Ìö® Îß§ÏπòÎ°?    // ?òÏñ¥?§Î?Î°? ?ÅÌíà ?ÑÌôò ?úÏ†ê Î¶¨ÏÖã??Î≥ëÌñâ?úÎã§.
    setSelectedOption(product.options?.[0]?.id || '');
  }
  // brandName ?Ä repo Í∞Ä Ï°∞Ïù∏???¥Î†§Ï§Ä??ÏΩòÏÑº????src/types/index.ts Product.brandName).
  const brandName = product.brandName ?? product.brandId;


  // ?µÏÖò?Ä ?ÅÌÉúÎ•?ÎØøÏ? ?äÍ≥† Îß??åÎçî Í≤ÄÏ¶????ÑÏû¨ ?ÅÌíà???ÜÎäî ?µÏÖò ID??Ï≤??µÏÖò?ºÎ°ú ?ÄÏ≤?  const validOption = product.options?.find(o => o.id === selectedOption) ?? product.options?.[0];
  ...
  // ?úÏãú¬∑Í≥ÑÏÇ∞¬∑?∏Îì§???ÑÎã¨ ?òÎüâ ?ºÏõê????stock Î≥Ä?ôÍ≥º Î¨¥Í??òÍ≤å ??ÉÅ 1 ?¥ÏÉÅ?ºÎ°ú ?¥Îû®??  const displayQty = Math.max(1, Math.min(quantity, Math.max(1, product.stock)));
  ...
  const unavailableDescription = isAdminViewer
    ? 'Í¥ÄÎ¶¨Ïûê ?ÅÌíà ?∏Ïßë?êÏÑú ?êÎß§Í∞Ä?Ä ?¨Í≥†Î•??ÖÎ†•?òÎ©¥ ?•Î∞îÍµ¨Îãà?Ä Î∞îÎ°úÍµ¨Îß§Í∞Ä ?úÏÑ±?îÎê©?àÎã§.'
    : '?êÎß§Í∞ÄÍ∞Ä ?ïÏ†ï?òÎ©¥ ?•Î∞îÍµ¨Îãà?Ä Î∞îÎ°úÍµ¨Îß§Î•??¥Ïö©?????àÏäµ?àÎã§.';
  const adminEditLabel = hasPrice ? 'Í¥ÄÎ¶¨Ïûê ?ÅÌíà ?ïÎ≥¥ ?òÏ†ï' : 'Í¥ÄÎ¶¨Ïûê?êÏÑú ?êÎß§Í∞Ä ?ÖÎ†•';
  // Î∞©Ïñ¥???∏Îç±???¥Îû®????gallery Ï∂ïÏÜå(?ÅÌíà ?ÑÌôò ÏßÅÌõÑ ?åÎçî) ??undefined src Î∞©Ï?
  ...
  const handleAddToCart = () => {
    if (!hasPrice) {
      alert('Í∞ÄÍ≤©ÏùÑ Î®ºÏ? ?ïÏù∏?¥Ï£º?∏Ïöî.');
      router.push('/login');
      return;
    }
    if (product.stock <= 0) {
      alert('?ºÏãú ?àÏ†à???ÅÌíà?ÖÎãà??');
      return;
    }
    addToCart({
    ...
    <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
              ...
              <div className="mt-4 flex gap-4 overflow-x-auto hide-scrollbar pb-2">
                {gallery.map((src, i) => (
                  <button
                    ...
                    aria-label={`${product.name} ?¥Î?ÏßÄ ${i + 1}`}
      ...
      <div className="w-full lg:w-1/2 flex flex-col pt-2">
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-[#17211D]">
          ...
          <span className="text-[#8A918B] ml-1">Íµ¨Îß§??{product.reviewCount}Í∞?/span>
        </div>
        ...
        <div className="mt-8 space-y-4 text-sm">
          <div className="flex">
            <span className="w-24 text-[#6F766F] font-medium">Î∞∞ÏÜ°Îπ?/span>
            <span className="text-[#6F766F]">
              {product.shippingFee !== undefined
                ? `${formatPrice(product.shippingFee)} (50,000???¥ÏÉÅ Î¨¥Î£åÎ∞∞ÏÜ°)`
                : DEFAULT_COMMERCE_POLICY.shippingLabel}
            </span>
          </div>
          {pointsRateLabel && (
            <div className="flex">
              <span className="w-24 text-[#6F766F] font-medium">?ÅÎ¶ΩÍ∏?/span>
              <span className="text-[#6F766F]">?ÅÌíàÍ∏àÏï° Í∏∞Ï? {pointsRateLabel} ?ÅÎ¶Ω ?§Ï†ï</span>
            </div>
          )}
        </div>
        ...
        {product.options && product.options.length > 0 && (
          <div className="mt-10">
            <label className="block text-sm font-semibold text-[#17211D] mb-3">?µÏÖò ?†ÌÉù</label>
        ...
        {hasPrice && (
          <div className="mt-8 flex items-end justify-between pt-6 border-t border-[rgba(15,23,42,0.06)]">
            <span className="text-base font-semibold text-[#6F766F]">Ï¥??ÅÌíàÍ∏àÏï°</span>
        ...
        <div className="mt-6 md:mt-8 flex gap-2 md:gap-3">
          <button
            type="button"
            aria-label={wishlisted ? `${product.name} Ï∞??¥Ï†ú` : `${product.name} Ï∞úÌïòÍ∏?}
          ...
          {hasPrice ? (
            <>
              ...
              >
                <ShoppingCart className="mr-1.5 md:mr-2 h-4 w-4 md:h-5 md:w-5" /> {isSellable ? '?•Î∞îÍµ¨Îãà' : '?àÏ†à'}
              </button>
              <button
                type="button"
              ...
              >
                <CreditCard className="mr-1.5 md:mr-2 h-4 w-4 md:h-5 md:w-5" /> {isSellable ? 'Î∞îÎ°úÍµ¨Îß§' : '?àÏ†à'}
              </button>
            </>
          ) : (
            <>
              ...
              >
                <ShoppingCart className="mr-1.5 md:mr-2 h-4 w-4 md:h-5 md:w-5" /> ?•Î∞îÍµ¨Îãà Ï§ÄÎπÑÏ§ë
              </button>
              <button
                type="button"
                disabled
              ...
              >
                <CreditCard className="mr-1.5 md:mr-2 h-4 w-4 md:h-5 md:w-5" /> Í≤∞Ï†ú Ï§ÄÎπÑÏ§ë
              </button>
            </>
          )}
        </div>
