...
4. **main 보호와 충돌을 존중한다.** `SESSION.md`가 직접 갱신 가능한 브랜치에 있으면 수정하고, main 보호/충돌/권한 때문에 직접 반영할 수 없으면 갱신용 diff 또는 별도 브랜치/PR 경로를 남긴다. 기존 세션 기록은 삭제하지 말고 최신 상태를 위쪽에 누적한다.
