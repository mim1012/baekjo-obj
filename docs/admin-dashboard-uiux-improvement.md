...
1. ~~**"dead mock 4종 삭제"는 틀렸다.** `src/data/products.ts`·`brands.ts`는 import가 0건이지만 **재시드의 정본**이다 — AGENTS.md 데이터 오너십 표 + 마이그레이션 `0004b`·`0014`~`0018`이 "정본 `src/data/*.ts` 기준"이라 주석에 명시 + `eslint.config.mjs`의 `no-restricted-imports` 대상 + `generate_placeholders.mjs`가 읽는다. **삭제 가능한 것은 `orders.ts`·`insuranceApplications.ts`·`users.ts` 3개뿐**(#51에서 처리).~~
   → ⚠️ **2026-07-17 이 정정을 다시 뒤집었다** (`be/kill-static-seed-canon`). 위 문단이 삭제 금지의 근거로 든 "재시드의 정본"이라는 지위가 **드리프트의 원인 그 자체**였다. 화면은 이미 DB만 읽는데(런타임 import 0건) 같은 값이 ① `src/data/*.ts` ② 손으로 타이핑한 시드 SQL(`0004b`) ③ `/admin`이 실시간 수정하는 라이브 DB — **세 곳에 각각 손으로** 적히고 있었고, 셋을 맞추는 건 사람의 기억력뿐이었다. 그 실패의 영수증이 정적↔DB 불일치 수습용 마이그레이션 `0014`~`0018`과, 프로덕션 값을 파일로 **역기입**한 `0035`다. lint 규칙은 *읽기 경로*만 막았기 때문에 드리프트를 화면 레이어에서 **시드 레이어로 이사**시켰을 뿐이다(거긴 테스트도 CI도 없다). DB 재구축 경로는 파일이 아니라 `supabase/migrations/`(`0004b` + `0018` + 정정분 → `node scripts/apply-migrations.mjs`)이므로 파일이 유일한 복구 수단이 아니었다. `generate_placeholders.mjs`는 실사진 webp 도입으로 이미 죽은 부트스트랩 스크립트라 함께 삭제했다. **결론: 아래 (c) 표의 "개편 후 → 삭제"가 옳았다.** products·brands의 값 입력구는 이제 `/admin` 하나뿐이다.
...
| 파일 | 지금 | 개편 후 |
|---|---|---|
| `src/data/products.ts` · `brands.ts` · `orders.ts` · `insuranceApplications.ts` | **import 0건 (dead)** | ✅ **삭제 완료** — `orders`·`insuranceApplications`는 #51, `products`·`brands`는 `be/kill-static-seed-canon`(2026-07-17) |
