...
11. PR4 = [#337](https://github.com/mim1012/baekjo-obj/pull/337) → develop 병합 완료 2026-09-15 17:07Z, 머지 커밋 b6186ef(커밋 9f65fd1→ea40748→98becc1, CI verify/admin-contract/crud/payments-routes/products-contract/partners-contract/visual 통과).

## 2026-09-15 후속 5 (PR5 세션 버전·회원 목록 페이지네이션·환불 재요청 방어)

1. 워크트리 `D:\Project\BAGJO1-cms-integration-20260915`, 브랜치 `codex/session-members-refund-20260915`(PR4 병합 후 develop b6186ef 위로 rebase). 커밋: 세션 버전·회원 페이지네이션·환불 멱등키 가드 이식 → 회원 목록 로딩 게이트(리뷰 B1) → 인라인 에러 배너·폴백 로그 warn(리뷰 MEDIUM 2건) → 도달 불가 분기 제거(리뷰 MEDIUM) → 비밀번호 변경 골든 동의 필드. 계획 `.omx/plans/cms-pr5-session-members-refund-20260915.md`.
2. 계약 요약: (a) `members.session_version`(0172, 기본 0)과 트리거 `members_session_version_bump`(password_hash·role 변경, status→inactive/withdrawn/rejected 시 +1). Auth.js jwt 콜백이 재검증 경로에서 DB 버전을 다시 읽어 `isCurrentSession`(active + 안전 정수 + 정확 일치)이 아니면 null을 돌려 세션을 끊는다. 0172 미적용 배포에서는 42703 폴백으로 레거시 조회(버전 0)하며 `logServerWarn` 1회 기록. `src/proxy.ts`는 `auth.config`만 쓰던 `NextAuth(authConfig)` 대신 `@/lib/auth`의 `auth()`를 써 관리자/마이페이지 가드에서도 DB 재검증이 적용된다. (b) `list_admin_member_page(p_page, p_page_size, p_search, p_role, p_status)`(0173, jsonb_build_object 화이트리스트)로 `/api/admin/members`가 페이지당 20건 서버 페이지네이션(`?search=`·`role`·`status`·`page`), 잘못된 질의는 PT409→400. 회원 목록 UI는 최초 로드에만 전체 로딩, 이후 조회 실패는 검색 입력창을 유지한 채 표 자리에 인라인 ErrorState(재시도). (c) `create_order_refund_request`(0174 재정의)가 같은 멱등키의 기존 원장을 재사용할 때 주문 불일치·종결 상태를 PT409로 막고, 환불 라우트는 `assertRefundableOrder`→`normalizeRefundPayload` 순으로 재요청을 방어.
3. 독립 리뷰(작성자 분리): 417c590 FAIL 1건(B1 회원 목록 전체 로딩이 검색 입력창 언마운트) → 1f43448 **PASS**(MEDIUM 2: 폴백 로그 error 레벨 알람 피로, 에러 early-return도 같은 클래스) → cb5e916 PASS(MEDIUM 1: 도달 불가능한 전체 페이지 ErrorState 분기) → 29ca958 PASS(신규 지적 0). 증거 `.omx/review-evidence/{1f43448,cb5e916,29ca958}/`.
4. staging 적용(불변): **0172(2026-09-15 16:23:05Z, sha256 363c732c…) 0173(16:23:06Z, 9b5096df…) 0174(16:23:07Z, 32136a39…)**, 객체 확인(컬럼 int default 0·트리거·함수 시그니처·PT409 가드 포함) 후 PostgREST 스키마 리로드. 범용 적용기 `artifacts/apply-migration.mjs <file> [--apply]`(dry-run 기본, `_migrations` 기록 존재 시 무시). 이후 SQL은 0175+.
5. 순수 게이트: tsc 0, admin+security 678 passed(리뷰어 재실행 동일 수치), rebase 후 전 계약 스위트 재실행(아래 7항).
6. 라이브 검증(로컬 3130 + staging DB, 0172~0174 적용 후): (a) API 흐름 — csrf→`callback/credentials`→`/api/members/me` 200; 골든이 만든 회원으로 `PATCH /api/members/password` 200 → `session_version` 1→2 → **같은 쿠키로 `/api/auth/session` null·`/api/members/me` `{"user":null}`** → 이전 비밀번호 로그인 `CredentialsSignin` 거부, 새 비밀번호 로그인 성공(세션 폐기 계약 실증). 골든 `member-password-change.spec.ts` 실행 중 UI 비밀번호 변경으로 트리거가 0→1로 올린 것도 DB에서 확인. (b) `tests/payments/partial-refund.db.spec.ts` **4/4 PASS**(0174로 재정의된 `create_order_refund_request` 경유). (c) `/api/admin/members?search=<email>` 페이지네이션 RPC 경로가 골든 셋업 단계에서 통과.
7. 골든 실구동 결과와 한계: `admin-crud-members.spec.ts`는 고정 계정이 pending이 아니라 설계대로 skip. `admin-crud-members-pending-signup.spec.ts`(B2B 탭 클릭 무시)·`credentials-login.spec.ts`(로그인 버튼이 네이티브 submit으로 `/login?` 이동)·`member-password-change.spec.ts`(이전 비밀번호 오류 문구 대기 단계에서 같은 네이티브 submit)는 **PR4 서버(3131, PR5 코드 없음)에서도 동일하게 실패**해 dev 모드(main-app.js 13MB) 하이드레이션 전 클릭 플레이크로 판정 — PR5 회귀 아님(프록시 matcher가 /login·/signup을 제외하고 두 서버 응답 시간도 동일). 이 세 스펙은 CI Preview(프로덕션 빌드)에서 검증한다. `member-password-change.spec.ts`의 가입 요청은 0150 동의 드리프트(termsAgree/privacyAgree 없으면 400)라 스펙만 보강.
8. 배포 메모: Production은 0162~0171 적용 후 **0172를 먼저** 적용한다(0172 적용 순간 기존 JWT는 `sessionVersion` 필드가 없어 다음 요청에서 무효 → 전 회원 재로그인 1회 발생; 적용 전에는 42703 폴백으로 무해). 0173은 관리자 회원 목록 API가 RPC에 의존하므로 PR5 코드 배포와 같은 릴리스에 묶는다. 0174는 함수 재정의만이라 순서 제약 없음.

## 2026-09-16 후속 6 (CMS 편집기 노출 필드 ↔ 렌더러 소비 필드 정합)

배경: 감사 결과 "관리자 편집기(CmsPageDefinition 섹션 fields)에 뜨는데 공개 렌더러가 안 읽는 필드"가 다수 확인됨. 사용자 요구="관리자에 보이는 설정값 = 렌더러가 모두 소비". PR = [#339](https://github.com/mim1012/baekjo-obj/pull/339) `codex/cms-editable-field-parity-20260916`(develop 919dbc2 기준, 커밋 a1d8ab0).

1. 제거(죽은/중복 노브): privacy 표 11섹션(66필드, 내용이 privacy 16조항 articles에 중복·StaticLegalDocument는 articles만 렌더), home solutions 섹션(page.tsx가 void solutions로 드롭), concerns insurance.visible(FEATURES.insurance 게이트), site-shell company 5필드(서비스명·이메일·운영시간·개인정보책임자·호스팅 — 미소비, 법무 {{company.*}} 토큰은 COMPANY 상수 치환이라 무관). site-shell 5필드는 PR #334에서 정의만 넣고 렌더 연결 안 한 것(git blame e3b352b 확인).
2. 연결(편집값이 화면에 반영, 선택 요소는 값 있을 때만 조건부): home audit.badge·hero.badge·진단 링크(homeContent 기본값 있어 홈 기본 노출 — 사용자 승인), 법무 companyBoxTitle·footerNote, brands standards.items[].visible 필터, b2b programs 카드 image/imageAlt(resolveCmsImageProps), experts 2차 CTA·프로세스 설명, insurance-landing 히어로 이미지·2차 CTA.
3. 검증: tsc 0, admin+products+security 순수 계약 1158(+company 제거 후 1093 재확인). 독립 리뷰 PASS(.omx/review-evidence/parity-20260916/review.md). 실구동(로컬 3130+staging): 공개 페이지 전부 200(experts·landing/insurance는 기능 플래그 OFF 기존 307), 홈 배지·진단 링크 실렌더, privacy 조항 본문 유지(표 제거로 콘텐츠 손실 없음).
4. 범위 밖(별도 gap): /insurance 메인·apply·complete·recommend·concerns 상세 슬러그별 카피는 CMS 밖 하드코딩(감사로 확인). normalize.ts가 정의 밖 필드를 무시하므로 과거 게시본은 안 깨짐.
