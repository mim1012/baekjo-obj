...
6. 미완료 게이트: `tests/payments/action-request.db.spec.ts`(staging 쓰기 필요, 이 세션에서는 미실행), 골든 실구동 스펙 `tests/golden/admin-crud-order-cancellation.spec.ts`(UI 담당 에이전트 작업 중 — 이 시점 워크트리에 파일이 아직 없어 `tests/admin/golden-crud-coverage.spec.ts`의 "모든 admin-crud-*·member-* 파일이 yml에 등록" 검사가 이 파일을 아직 대상으로 인식하지 못함, 파일 생성 후 재검증 필요), 그리고 0169~0171 마이그레이션·U3 서버 로직에 대한 독립 리뷰가 남아 있다.
