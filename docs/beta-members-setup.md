...
- **비밀번호 변경 후 기존 세션 유지**: JWT 무상태 세션이라 비밀번호를 바꿔도 이미 발급된 토큰은 만료까지 유효(탈취 의심 시나리오에서 한계). 근본 해결은 jwt 콜백에 passwordChangedAt 검사 도입 — Phase 2. 비밀번호 변경 엔드포인트도 rate limit 미적용(위 항목과 동일 게이트).
