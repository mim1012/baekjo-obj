...
// ★2026-07-13 R4 라운드2 추가: confirmPayment.ts ③-a(클레임 전 바인딩 검증, queryTossPayment)가
// claim보다 먼저 실행되게 바뀌었다. 이 프리뷰 환경은 TOSS_SECRET_KEY가 없어 그 바인딩 조회
// 자체가 항상 "불명"으로 실패하므로, 아래 세 번째 테스트는 이제 claim에 도달하지 못하고
// 주문이 결제대기에 남는다(실배포에서 진짜 paymentKey가 바인딩 검증을 통과하면 여전히 claim →
// 승인중으로 전이한다 — 이 describe의 제목·위 설명은 그 일반 경로를 설명한 것이고, 프리뷰의
// 시크릿 미설정이라는 조건에서만 claim 도달 전에 끊긴다).
test.describe.serial('결제 라우트 — 주문 선점/불명 상태(claim 잔존) (프리뷰 통합)', () => {
  ...
  test('불명 상태(TOSS_SECRET_KEY 미설정)에서는 502를 반환하고, claim 이전이므로 결제대기·재고를 그대로 둔다', async () => {
    // 2026-07-13 R4 라운드2(바인딩 검증 선행) 이후 동작 변경: claim보다 먼저 queryTossPayment로
    // paymentKey↔orderId·금액 바인딩을 확인한다(confirmPayment.ts ③-a). 이 프리뷰 환경은
    // TOSS_SECRET_KEY가 없어 그 조회 자체가 "불명"(httpStatus=null)으로 실패하므로, claim은
    // 아예 호출되지 않는다 — 예전엔 claim이 먼저 실행돼 결제대기→승인중으로 전이한 뒤 토스
    // confirm이 실패해 주문이 "승인중"에 잔존했지만, 이제는 claim 이전에 끊기므로 주문은
    // 결제대기 그대로 남는다(더 안전함 — 승인중은 이 공개 API로 취소 불가능한 상태다).
...
test.describe('결제 라우트 — GET /api/payments/return 바인딩 검증 + status 읽기전용 (R4 라운드2, 프리뷰 통합)', () => {
  ...
  test('위조된 paymentKey로 /api/payments/return을 호출해도 결제대기 주문·재고를 건드리지 않는다', async () => {
    ...
    // 이 프리뷰 환경은 TOSS_SECRET_KEY 미설정이라 바인딩 조회 자체가 "불명"(httpStatus=null)으로
    // 떨어진다(진짜 토스 배포라면 404로 명확히 갈린다) — 어느 쪽이든 claim은 절대 실행되지
    // 않는다는 게 핵심이며, 아래 DB 단언이 그 회귀 방어다.
    expect(location).toMatch(/status=unconfirmed/);
    const row = await orderRow(orderId);
