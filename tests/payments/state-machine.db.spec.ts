...
test.describe.serial('결제 상태기계 — claim 배타 전이 / 0024·0028 상호배타', () => {
  ...
  test('0028은 결제완료 주문을 보호하고, payment_key가 일치해야만 승인중 주문을 해제·재고를 복원한다', async () => {
    ...
    const confirmedOk = (await q(`select public.cancel_confirming_and_restore('${claimedOrderId}', '__test_tk_claim') as ok;`))[0].ok;
    ...
    const wrongKeyOk = (await q(`select public.cancel_confirming_and_restore('${confirmingOrderId}', '__test_tk_wrong_key') as ok;`))[0].ok;
    ...
    const confirmingOk = (await q(`select public.cancel_confirming_and_restore('${confirmingOrderId}', '__test_tk_confirming') as ok;`))[0].ok;
    ...
    const secondCall = (await q(`select public.cancel_confirming_and_restore('${confirmingOrderId}', '__test_tk_confirming') as ok;`))[0].ok;
    expect(secondCall).toBe(false);
    expect(await stockOf(P)).toBe(7);
  });

  test('0028은 결제대기 주문을 건드리지 않는다 (0024와 상호배타)', async () => {
    const ok = (await q(`select public.cancel_confirming_and_restore('${pendingOrderId}', '__test_tk_irrelevant') as ok;`))[0].ok;
    expect(ok).toBe(false);
  });
