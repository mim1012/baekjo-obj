...
test('고민 상세는 추천 브랜드 구역과 브랜드 조회 기능을 포함하지 않는다', () => {
  ...
  const defaults = fs.readFileSync(path.join(root, 'src', 'lib', 'concerns', 'config.ts'), 'utf8');
  ...
  expect(defaults).toContain('일상 관리에 함께 볼 상품');
  expect(defaults).toContain('많이 궁금해하시는 점');
  expect(source).toContain('concern.productsTitle');
  expect(source).toContain('concern.faqTitle');
