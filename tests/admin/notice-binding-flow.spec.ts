...
test.describe('공지사항(notices) 관리자 저장 → 공개 화면 바인딩 경로', () => {
  test('관리자 공지 화면은 storage 콘센트와 CRUD 콜백을 모두 연결한다', () => {
    ...
    expect(pageSource).toContain('const persistedItemsRef = useRef<Notice[]>([]);');
    expect(pageSource).toContain('const nextItems = [...persistedItemsRef.current, newNotice];');
    expect(pageSource).toContain('const nextItems = persistedItemsRef.current.map((notice) =>');
    expect(pageSource).toContain('const nextItems = persistedItemsRef.current.filter((notice) => notice.id !== id);');
    expect((pageSource.match(/saveNoticesConfig\(\{ items: nextItems \}\)/g) ?? []).length).toBe(3);
    ...
    expect(pageSource).not.toContain('마지막 공지는 삭제할 수 없습니다.');
