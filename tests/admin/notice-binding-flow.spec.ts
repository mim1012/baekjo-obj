...
test.describe('공지사항(notices) 관리자 저장 → 공개 화면 바인딩 경로', () => {
  test('관리자 공지 화면은 storage 콘센트와 CRUD 콜백을 모두 연결한다', () => {
    ...
    expect(pageSource).toContain('const persistedItemsRef = useRef<Notice[]>(defaultNoticesConfig.items);');
    expect(pageSource).toContain('const nextItems = persistedItemsRef.current.filter((notice) => notice.id !== id);');
    expect(pageSource).toContain('saveNoticesConfig({ items: nextItems })');
