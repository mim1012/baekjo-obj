...
test.describe('관리자 주문 export API 계약', () => {
  test('신규 export route는 관리자 인증, 서버 DB 조회, 공용 필터, XLSX 직렬화를 모두 사용한다', () => {
    ...
    expect(route).toContain('serializeAdminOrdersXlsx({');
    expect(route).toContain("'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'");
    expect(route).toContain("'Content-Disposition': 'attachment; filename=\"admin-orders-export.xlsx\"'");
    expect(route).not.toContain('requester');
    expect(route).toContain('brandId: parsed.filters.brandId');
  });
