...
test.describe('전 페이지 스모크 검수 커버리지 감사 — 새 page.tsx 누락 방지', () => {
  ...
  // ── 관리자 페이지 음성 케이스(비로그인 차단) 커버리지 ─────────────────────────
  // 별도 레지스트리를 두지 않는다 — all-pages-smoke.spec.ts 가 ALL_APP_ROUTES 를 auth==='admin' 로
  // 필터링해 전부 순회하므로, 위 두 테스트(신규/삭제 페이지 감지)가 이미 이 커버리지를 보장한다.
  // 새 admin 페이지를 ALL_APP_ROUTES 에 등록하면 자동으로 비로그인 차단 검사 대상이 된다.
  test('ALL_APP_ROUTES 의 admin 라우트는 전부 auth 값이 정확히 "admin" 이다(오분류 방지)', () => {
    // /admin 으로 시작하는데 auth 가 admin이 아니면(오타 등) 비로그인 차단 검사에서 누락된다.
    const misclassified = ALL_APP_ROUTES.filter(
      (r) => r.route.startsWith('/admin') && r.auth !== 'admin',
    ).map((r) => r.route);
    expect(misclassified).toEqual([]);
  });
});

test.describe('관리자 API 비로그인/비관리자 차단 커버리지 감사 — 새 route.ts 누락 방지', () => {
  /** src/app/api/admin 아래 모든 route.ts 를 재귀 탐색해 라우트 문자열로 변환한다. */
  function listAdminApiRoutes(dir: string, base = '/api/admin'): string[] {
    const routes: string[] = [];
    if (fs.existsSync(path.join(dir, 'route.ts'))) routes.push(base);
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      routes.push(...listAdminApiRoutes(path.join(dir, entry.name), `${base}/${entry.name}`));
    }
    return routes.sort();
  }
  ...
  test('src/app/api/admin 의 모든 route.ts 가 ALL_ADMIN_API_ROUTES(비로그인/비관리자 차단 검수 대상)에 등록돼 있다', () => {
    const actual = listAdminApiRoutes(ADMIN_API_DIR);
    const registered = new Set(ALL_ADMIN_API_ROUTES);
    const missing = actual.filter((route) => !registered.has(route));
    expect(
      missing,
      `신규 관리자 API [${missing.join(', ')}] 가 tests/golden/_lib/allAdminApiRoutes.ts 의 ` +
        'ALL_ADMIN_API_ROUTES 에 없습니다 — 등록하면 all-pages-smoke.spec.ts 가 자동으로 비로그인(401)· ' +
        '비관리자(403) 차단을 검수합니다.',
    ).toEqual([]);
  });

  test('ALL_ADMIN_API_ROUTES 의 모든 라우트가 여전히 실존하는 route.ts 를 가리킨다', () => {
    const actual = new Set(listAdminApiRoutes(ADMIN_API_DIR));
    const stale = ALL_ADMIN_API_ROUTES.filter((route) => !actual.has(route));
    expect(
      stale,
      `ALL_ADMIN_API_ROUTES 에 있지만 더 이상 존재하지 않는 라우트: [${stale.join(', ')}] — ` +
        '삭제됐다면 allAdminApiRoutes.ts 에서도 정리하세요.',
    ).toEqual([]);
  });

  test('ALL_ADMIN_API_ROUTES 안에 중복 라우트가 없다', () => {
    const duplicates = ALL_ADMIN_API_ROUTES.filter(
      (route, index) => ALL_ADMIN_API_ROUTES.indexOf(route) !== index,
    );
    expect(duplicates).toEqual([]);
  });
});
