...
test.describe('파트너/케어키트 관리자 저장 → 공개 랜딩 바인딩 경로', () => {
  test('제휴처 관리자 저장은 storage 콘센트와 admin partners API/repo readback 경로를 사용한다', () => {
    ...
    // wave-4 수정(2026-07-19) — content-binding-flow.spec.ts와 동일하게 새 조건으로 갱신.
    ...
    expect(repoSource).toContain("const PARTNER_TYPES: Partner['type'][] = ['hospital', 'funeral', 'brand', 'hotel', 'etc'];");
    expect(repoSource).toContain('function normalizePartnerType(type: unknown): Partner[\'type\']');
  ...
  test('제휴처 등록·수정·삭제 모두 persisted 기준으로 즉시 저장한다(빈 배열 허용 — 마지막 항목 차단 없음)', () => {
    ...
    expect(pageSource).toContain('const persistedItemsRef = useRef<Partner[]>([]);');
