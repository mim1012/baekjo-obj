...
test.describe('파트너/케어키트 관리자 저장 → 공개 랜딩 바인딩 경로', () => {
  test('제휴처 관리자 저장은 storage 콘센트와 admin partners API/repo readback 경로를 사용한다', () => {
    ...
    const saveHandler = sliceBetween(pageSource, 'const handleSave = async () =>', 'return (');
    ...
    expect(saveHandler).toContain('if (!loaded || loadError || busyRef.current) return { ok: false };');
    ...
    // 헤더 저장이 진행 중(saving)이면 행 삭제 버튼도 비활성화한다 — 페이지 레벨 busyRef 가드의
    // UX 보완(codex 2차 리뷰 HIGH).
  ...
  test('제휴처 삭제는 persisted 기준으로 즉시 저장한다(빈 배열 허용 — 마지막 항목 차단 없음)', () => {
    const pageSource = src('src', 'app', 'admin', 'partners', 'page.tsx');
  ...
  test('케어키트 삭제는 persisted 기준으로 즉시 저장한다(빈 배열 허용 — 마지막 항목 차단 없음)', () => {
    const pageSource = src('src', 'app', 'admin', 'kits', 'page.tsx');
