...
test.describe('파트너/케어키트 관리자 저장 → 공개 랜딩 바인딩 경로', () => {
  test('제휴처 관리자 저장은 storage 콘센트와 admin partners API/repo readback 경로를 사용한다', () => {
    ...
    expect(pageSource).not.toContain('createFields=');
    expect(pageSource).not.toContain('제휴처 등록');
    expect(pageSource).toContain('disableEdit');
    expect(resourcePage).toContain('disableEdit?: boolean;');
    expect(resourcePage).toContain('const canEditRows = !readOnly && !disableEdit;');
    expect(resourcePage).toContain('{canEditRows && (');
    expect(resourcePage).toContain('{canDeleteRows && (');
  ...
  test('케어키트 관리자 저장은 storage 콘센트와 admin kits API/repo readback 경로를 사용한다', () => {
    ...
    expect(pageSource).not.toContain('actionLabel=');
    expect(pageSource).not.toContain('createFields=');
    expect(pageSource).not.toContain('키트 등록');
    expect(pageSource).toContain('disableEdit');
    expectNoMutableProductBrandImport(pageSource);
