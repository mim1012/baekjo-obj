...
test.describe('실제 고객 화면 ↔ 관리자 단일 연결 기준표', () => {
  ...
  test('게시형 관리 버튼은 실제 CMS 정의와 1:1로 연결된다', () => {
    ...
    // 보험 랜딩(insurance-landing)은 이번 관리자 개편 범위에서 명시적으로 제외한다.
  ...
  test('중복 공개 라우트가 없다', () => {
    const routes = PUBLIC_PAGE_REGISTRY.map((page) => page.routePattern);
    const duplicates = routes.filter((route, index) => routes.indexOf(route) !== index);
    expect(duplicates).toEqual([]);
  });

  test('레지스트리 key도 중복이 없다', () => {
    const keys = PUBLIC_PAGE_REGISTRY.map((page) => page.key);
