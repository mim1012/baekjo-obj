...
test.describe('전시용 후기(showcase reviews) 관리자 저장 → 공개 화면 바인딩 경로', () => {
  test('관리자 후기 화면은 storage 콘센트와 CRUD 콜백을 모두 연결한다', () => {
    ...
    expect(pageSource).toContain('saveShowcaseReviewsConfig({ items })');
    // 로드 완료 전·로드 실패 시 저장을 막는다 — default 로 DB 를 덮어쓰는 레이스 방지(codex F-HIGH, notices 미러).
    ...
    // 삭제는 batch save 를 기다리지 않고 즉시 저장한다(공지 삭제 오인 리포트와 동일 UX — notices 미러).
    expect(pageSource).toContain('saveShowcaseReviewsConfig({ items: nextItems })');
    // notices 와 달리 전시 후기는 빈 목록을 허용 — 마지막 1건 삭제를 막는 가드를 두지 않는다.
    expect(pageSource).not.toContain('nextItems.length === 0');
    // 삭제는 persisted 기준(마지막으로 DB 와 일치한 목록)으로 계산한다 — 미저장 등록·수정 드래프트가
    // 삭제에 딸려 커밋되지 않는다(opus 리뷰 MEDIUM-1).
    expect(pageSource).toContain('const persistedItemsRef = useRef<Review[]>(defaultShowcaseReviewsConfig.items);');
    ...
    // 저장·삭제 공용 상호배제 — 저장 PUT 진행 중 삭제가 끼어들어 방금 지운 항목을 되살리는 레이스를
    // 막는다(codex 2차 리뷰 HIGH). handleDelete·handleSave 양쪽 모두 busyRef 로 게이팅한다.
    expect(pageSource).toContain('const busyRef = useRef(false);');
    expect(pageSource).toContain('if (!loaded || loadError || busyRef.current) return;');
    expect(pageSource).toContain('if (!loaded || loadError || busyRef.current) return { ok: false };');
    expect((pageSource.match(/busyRef\.current = true;/g) ?? []).length).toBe(2);
    expect((pageSource.match(/busyRef\.current = false;/g) ?? []).length).toBe(2);
