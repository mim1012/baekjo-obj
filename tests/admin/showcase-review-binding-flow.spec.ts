...
test.describe('전시용 후기(showcase reviews) 관리자 저장 → 공개 화면 바인딩 경로', () => {
  test('관리자 후기 화면은 storage 콘센트와 CRUD 콜백을 모두 연결한다', () => {
    ...
    expect(pageSource).toContain('if (!loaded || loadError) return { ok: false };');
    ...
    // 삭제 재진입 가드 — 실패한 첫 삭제가 두 번째 삭제와 경합하지 않는다(opus 리뷰 LOW-1).
    expect(pageSource).toContain('const deletingRef = useRef(false);');
    expect(pageSource).toContain('if (!loaded || loadError || deletingRef.current) return;');
    expect(pageSource).toContain('deletingRef.current = true;');
    expect(pageSource).toContain('deletingRef.current = false;');
  });
