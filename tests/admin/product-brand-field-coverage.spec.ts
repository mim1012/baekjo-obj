...
const PRODUCT_EXCLUDED: Record<string, string> = {
  ...
  // ── 죽은 필드(dead field): validate/저장은 되지만 공개 소비자·관리자 UI가 전혀 없음 ──
  // (Explore 검증 2026-07-19: Product.X 를 렌더하는 공개 경로 0건. grep 히트는 전부 Brand의
  //  같은 이름 필드를 읽는 것. Brand.auditPoints/relatedConcernSlugs 는 실제 공개 소비된다.)
  auditPoints:
    'dead field — validate/저장되지만 공개 소비자 0건·관리자 UI 미노출. /shop/[id] #standard 는 ' +
    'brand.auditPoints 를 쓴다(product 의 것이 아님). 가짜 assertion 을 만들지 않는다.',
  relatedConcernSlugs:
    'dead field — validate/저장되지만 공개 소비자 0건·관리자 UI 미노출. Brand.relatedConcernSlugs ' +
    '(브랜드 상세에서 소비됨)와 다른 필드. 가짜 assertion 을 만들지 않는다.',
  tags:
    'dead field — validate/저장되지만 공개 소비자 0건·관리자 UI 미노출. 렌더 사이트 없음.',
};
...
test.describe('상품·브랜드 폼 필드 커버리지 감사 — 필드 누락 방지', () => {
  ...
  test('죽은 필드(dead field) 3종이 명시적으로 EXCLUDED 사유에 "dead field" 로 기록돼 있다', () => {
    // team-lead 요청: 이 3개는 가짜 assertion 을 만들지 말고 사유와 함께 제외로 못 박는다.
    for (const dead of ['auditPoints', 'relatedConcernSlugs', 'tags']) {
      expect(PRODUCT_EXCLUDED[dead], `Product.${dead} 는 EXCLUDED 사유가 있어야 함`).toBeTruthy();
      expect(PRODUCT_EXCLUDED[dead]).toContain('dead field');
    }
  });
});
