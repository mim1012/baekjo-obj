...
export const ALL_APP_ROUTES: RouteEntry[] = [
  ...
  { route: '/cart', kind: 'static', auth: 'redirect', expectedRedirect: '/login', note: '비로그인 장바구니 방문은 /login 으로 리다이렉트' },
  { route: '/concerns', kind: 'static', auth: 'none', note: '고민별 케어 목록' },
  { route: '/diagnosis', kind: 'static', auth: 'none', note: '1분 맞춤 진단 설문' },
  { route: '/experts', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 전문가 콘텐츠는 메인으로 리다이렉트' },
  { route: '/forgot-password', kind: 'static', auth: 'none', note: '비밀번호 찾기' },
  { route: '/insurance', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 펫보험 안내는 메인으로 리다이렉트' },
  { route: '/insurance/apply', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 보험 신청 폼은 메인으로 리다이렉트' },
  { route: '/insurance/complete', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 보험 완료 화면은 메인으로 리다이렉트' },
  { route: '/insurance/recommend', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 보험 추천 화면은 메인으로 리다이렉트' },
  { route: '/landing/care-kit', kind: 'static', auth: 'none', note: '케어키트 B2B 랜딩' },
  { route: '/landing/insurance', kind: 'static', auth: 'redirect', expectedRedirect: '/', note: '미노출 보험 랜딩은 메인으로 리다이렉트' },
