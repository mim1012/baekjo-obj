...
export const FIELD_SURFACE_MATRIX: Surface[] = [
  // ── 상품: shop 카드(검색/목록) ──────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { field: 'summary', render: 'src/components/common/ProductCard.tsx', kind: 'text', note: '상품 카드용 한 줄 설명.' },
      ...
      { field: 'isRecommended', render: 'src/components/shop/ShopContent.tsx', kind: 'derived', note: 'true→DAILY PICK 영역 포함.' },
      {
        field: 'stock',
        render: 'src/components/common/ProductCard.tsx:70,96-100',
        kind: 'derived',
        note: 'stock<=0→구매 버튼 비활성화, 가격 없음→"판매 준비 중".',
      },
    ],
  },
  ...
  // ── 상품: shop 상세 ───────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { field: 'detailBlocks', render: 'src/app/shop/[id]/page.tsx:70-89', kind: 'blocks', note: '#story 순서 보존(text→<p>, image→<img>).' },
      { field: 'description', render: 'src/app/shop/[id]/page.tsx:69', kind: 'text', note: '#story body-copy.' },
      ...
      { field: 'shippingNotice', render: 'src/components/shop/ProductPurchaseInfo.tsx:17-41', kind: 'text', note: '"배송 유의사항".' },
      ...
      { field: 'auditPoints', render: 'src/components/shop/ProductPublicDetails.tsx', kind: 'text', note: '상품 검증 포인트 목록.' },
      { field: 'ingredients', render: 'src/components/shop/ProductPublicDetails.tsx', kind: 'text', note: '성분/원재료.' },
      { field: 'howToUse', render: 'src/components/shop/ProductPublicDetails.tsx', kind: 'text', note: '급여/사용 방법.' },
      { field: 'recommendedFor', render: 'src/components/shop/ProductPublicDetails.tsx', kind: 'text', note: '추천 대상 목록.' },
      { field: 'caution', render: 'src/components/shop/ProductPublicDetails.tsx', kind: 'text', note: '주의사항 목록.' },
