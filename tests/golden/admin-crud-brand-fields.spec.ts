...
test.describe('골든플로우 #7: 관리자 CRUD 실구동 — 브랜드 전 필드 왕복(상세 에디터)', () => {
  ...
  test('상세 에디터 전 필드 등록 → 공개 반영 → 관리자 왕복 → 삭제', async ({ page }) => {
    ...
    // ── 1) 모달(BrandForm)로 브랜드 생성 — 공개 화면 필수값 + displayOrder=0 + isNew ──
    await page.goto('/admin/brands');
    ...
    await setProductDisplaySetting(page, prodName, 'visible', true);
    ...
    await expect(body, '공개 브랜드 상세에는 공식몰 방문 CTA를 렌더하지 않는다').not.toContainText('공식몰 방문하기');
