...
test.describe('골든플로우 #7: 관리자 CRUD 실구동 — 주문(안전 전이, 일회용 상품)', () => {
  ...
  test('일회용 상품 주문 → 무통장입금 → 입금확인(부정 케이스 포함) → 배송완료 전이 → 회원 반영 → 정리', async ({
    ...
    await expect(async () => {
      ...
      await expect(adminPage.locator('div.mb-6', { hasText: '배송 상태' }).locator('select')).toHaveValue(
        '배송완료',
        { timeout: 5_000 },
      );
    ...
    expect(finalOrder?.orderStatus).toBe('주문접수');
    ...
    await expect(memberPage.getByRole('button', { name: '구매평 작성' })).toHaveCount(0);
