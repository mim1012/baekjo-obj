...
test.describe('골든플로우: 관리자 구매평 검수(숨김→별점 제외→재노출→삭제)', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // 4) 회원: 구매평 작성(별점 4점) — member-review-inquiry.spec.ts와 동일한 UI 경로.
    // 구매확정 직후 mypage 읽기가 낡은 값을 줄 수 있어(당일 wishlist·shipments에서 재현된
    // 쓰기-직후-스테일 패턴) 재진입 폴링으로 감싼다.
    const writableCard = memberPage.locator('.mypage-card', { hasText: productName }).first();
    await expect(async () => {
      await memberPage.goto('/mypage?tab=reviews');
      await expect(writableCard).toBeVisible({ timeout: 5_000 });
    }).toPass({ timeout: 45_000 });
    await writableCard.getByRole('button', { name: '구매평 작성' }).click();
    await memberPage.getByRole('heading', { name: '구매평 작성' }).waitFor({ state: 'visible', timeout: 15_000 });
    await memberPage.locator('form button.p-1').nth(3).click(); // 왼쪽부터 4번째 = 4점
    await memberPage.locator('textarea').fill(reviewContent);
    await memberPage.getByRole('button', { name: '등록하기' }).click();
    await memberPage.getByRole('button', { name: /작성한 구매평/ }).click();
    await expect(memberPage.locator('.mypage-card', { hasText: reviewContent })).toBeVisible({ timeout: 15_000 });
