...
test.describe('골든플로우: 회원 여정 — 소셜 로그인 앱 내부 계약', () => {
  ...
  test('소셜 로그인 시작과 완료 브릿지는 /auth/complete → /api/members/me → setCurrentUser 로 이어진다', () => {
    ...
    expect(socialAuth).toContain('redirectTo: `${SOCIAL_COMPLETE_PATH}?${params.toString()}`');
    ...
    expect(completePage).toContain("const returnTo = normalizeReturnTo(new URLSearchParams(window.location.search).get('returnTo'));");
    expect(completePage).toContain('router.replace(user.profileCompleted ? returnTo : `/auth/complete-profile?returnTo=${encodeURIComponent(returnTo)}`);');
