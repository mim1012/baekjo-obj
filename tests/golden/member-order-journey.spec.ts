...
test.describe('골든플로우 #2: 회원 여정 — 스토어 구매(무통장입금) 실구동', () => {
  ...
  test('상품 2종 담기 → 장바구니 수량 변경 + 한 종 삭제 → 무통장 결제 → 완료 → 마이페이지 주문내역 반영', async ({
    ...
    await expect(page.locator('body')).toContainText('장바구니에 담겼습니다');
    ...
    // ⚠️ OrdersSection.tsx:151은 order.orderStatus 뱃지만 보여준다(paymentStatus는 렌더 안 함) —
    // 무통장입금 신규 주문은 orderStatus='주문접수'(paymentStatus='입금대기'는 이 화면에 노출되지
    // 않는 별도 필드, src/app/api/orders/route.ts:101,117).
