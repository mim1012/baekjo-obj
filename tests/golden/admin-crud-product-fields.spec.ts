...
//    dev 서버 기동 env 에 SUPABASE_URL 을 넣어야 한다
...
test.describe('골든플로우 #7: 관리자 CRUD 실구동 — 상품 폼 전 필드 왕복', () => {
  ...
  test('전 필드 등록 → 공개 반영 → detailBlocks → 관리자 왕복 → 경계/XSS 수정 → 삭제', async ({
    ...
    const optionSelect = page.locator('select').filter({ hasText: opt1Name }).first(); // options(select)
    ...
    const saveDetailResponsePromise = page.waitForResponse(
      (response) =>
        response.url().includes(`/api/admin/products/${encodeURIComponent(productId)}`) &&
        response.request().method() === 'PATCH',
      { timeout: 20_000 },
    );
    await page.getByRole('button', { name: '상세페이지 저장' }).click();
    const saveDetailResponse = await saveDetailResponsePromise;
    if (!saveDetailResponse.ok()) {
      throw new Error(
        `상세페이지 저장 실패: ${saveDetailResponse.status()} ${await saveDetailResponse.text()}`,
      );
    }

    // ── 12) 공개 #story — detailBlocks 가 순서대로(text → image) 렌더 ──
    await page.goto(`/shop/${productId}`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
    ...
    await expect(
      page.getByRole('group', { name: '스토어 카테고리 선택' }).getByRole('button', { name: categoryValue, exact: true }),
    ...
    await expect(page.getByRole('button', { name: '피부', exact: true })).toHaveAttribute('aria-pressed', 'true');
  ...
  test('빈 상태 변형 — 재고0 구매 비활성·옵션 없음·detailBlocks 없음 그레이스풀 렌더', async ({ page }) => {
    ...
    await expect(card).toContainText('잠시 품절');
    await expect(card.getByRole('button', { name: '잠시 품절' })).toBeDisabled();
    ...
    await page.goto(emptyDetailHref ?? `/shop?search=${encodeURIComponent(emptyName)}`, {
      waitUntil: 'domcontentloaded',
      timeout: 60_000,
    });
