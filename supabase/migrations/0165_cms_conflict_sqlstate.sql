-- PostgREST retries SQLSTATE 40001 (serialization_failure) transparently, so calls made through
-- /rest/v1/rpc/... (supabase-js) hang 30s+ on conflict instead of returning; PT409 is PostgREST's
-- non-retried custom-HTTP-status class, so these application-level conflicts now use it instead.
...
begin
  ...
  if v_page.published_content -> '__managedVersion' is distinct from '1'::jsonb then
    raise exception 'initial-import-required' using errcode = 'PT409';
  end if;
...
begin
  ...
  if not found
    ...
    raise exception 'audit-source-conflict' using errcode = 'PT409';
  end if;
  ...
  if v_page.published_content -> '__managedVersion' = '1'::jsonb then
    raise exception 'audit-already-managed' using errcode = 'PT409';
  end if;
  ...
  if v_page.draft_content is distinct from p_expected_content then
    raise exception 'audit-content-conflict' using errcode = 'PT409';
  end if;
